...
dist = [[[0]*(K+1) for i in range(M)] for j in range(N)]
dx = [-1,1,0,0]
dy = [0,0,1,-1]
def wall(graph, x1, y1, x2, y2, K, dist):
    queue = deque()
    ...
    while queue:
        ...
        for i in range(4):
            ...
            if 0<=nx<N and 0<=ny<M:  
                ...
                if z < K and graph[nx][ny] == 1 and dist[nx][ny][z+1] == 0: #(nx,ny)가 벽이고 아직 벽을 K번은 방문하지 않았으며 아직 방문하지 않은 곳이라면
                    dist[nx][ny][z+1] = dist[x][y][z] + 1
                    queue.append([nx,ny,z+1])
    return

wall(graph, 0, 0, N-1, M-1, K, dist)
ans = -1
for i in range(K+1):
    if dist[N-1][M-1][i] != 0:
